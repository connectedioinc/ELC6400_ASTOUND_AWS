...
class test_events_reporting(WrapTest):
    ...
    def test_custom_options_validations_saving(self):
        ...
        with self.subTest("main test"):
            ...
            event_section = self.get_section("event_juggler", sid)["values"]
            section = self.get_section("event_juggler", event_section["actions"][0])["values"]
            del section[".name"]
            self.assertEqual(section["smtp_email_group"], EMAIL_GROUP_NAME)

            x = self.put_data(f"{BASE_URL}/{sid}",
                              {"emailgroup": ""})
            x.assert_code(200)
            section = self.get_section("event_juggler", sid)["values"]
    ...
    def test_send_email_failure(self):
        ...
        with self.subTest("main test"):
            ...
            x.assert_error("event", "Email account configuration is invalid", ERR_CODES["EMAIL_GROUP_INVALID_CFG"])
    ...
    def test_events_reporting_daemon_is_running(self):
        ...
        with self.subTest("main test"):
            self.assertTrue(is_process_running(self.ssh, "event_juggler"), "Expected event_juggler to be running")
    ...
    def test_available_events_saving(self):
        ...
        with self.subTest("main test"):
            ...
            for chunk in self.chunks(events_data, 60):
                bulk = {"data": chunk}
