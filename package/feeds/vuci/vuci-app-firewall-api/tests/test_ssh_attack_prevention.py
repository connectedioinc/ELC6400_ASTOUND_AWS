...
class test_firewall_ssh_attack_prevention(util.WrapTest):
    url = "/attack_prevention/ssh/config"
    sid = "general"
    default_data = {
        ".type": "rule",
        "ssh_limit": "0"
    ...
    def test_firewall_ssh_attack_prevention_base_functionality(self):
    ...
    def test_firewall_ssh_attack_prevention_deletion(self):
        x = self.delete(self.url + "/" + self.sid)
        x.assert_error("Validation", "Section deletion is not allowed", 111, None, None)

    def test_firewall_ssh_attack_prevention_creation(self):
