...
class test_firewall_https_attack_prevention(util.WrapTest):
    url = "/attack_prevention/https/config"
    sid = "general"
    default_data = {
        ".type": "rule",
        "https_limit": "0"
    ...
    def test_firewall_https_attack_prevention_base_functionality(self):
    ...
    def test_firewall_https_attack_prevention_deletion(self):
        x = self.delete(self.url + "/" + self.sid)
        x.assert_error("Validation", "Section deletion is not allowed", 111, None, None)

    def test_firewall_https_attack_prevention_creation(self):
