...
local ModbusTcpClientAlarm = ConfigService:new({ increment_name = true })

local s = ModbusTcpClientAlarm:section("modbus_client", function(self) return "alarm_" .. self.binding  end)
function s:filter()
	local parent = self:table_get(self.config, self.binding)
	if parent[".type"] == "tcp_server" then return true end
end

-----------------------------------------------START OF OPTIONS---------------------------------------------------------------------------

	local enabled = s:option("enabled")
	enabled.require = { ["1"] = { "f_code", "data_type", "register", "value", "condition", "actionfrequency", "action" } }
		function enabled:validate(value)
			return self.dt:is_bool(value)
		end

	local _function = s:option("f_code")
	...
	local redundancy_protection = s:option("redundancy_protection")
		function redundancy_protection:validate(value)
			return self.dt:is_bool(value)
		end
	...
	action.require = {
		[ALARM_ACTION.MODBUS] = {
			"modbus_timeout", "modbus_id", "modbus_first_reg", "modbus_function", "modbus_data_type"
		},
	...
	local modbus_data_type = s:option("modbus_data_type")
		function modbus_data_type:validate(value)
			...
			local data_types = modbus_utils:get_data_types(f_code, false)
			return self.dt:check_array(value, data_types)
		end
	...
	local data_type = s:option("data_type")
		function data_type:validate(value)
			local func = self:get_abs_value(self.config, self.sid, "f_code")
			local data_types = modbus_utils:get_data_types(func, false)
			return self.dt:check_array(value, data_types)
		end
...
function ModbusTcpClientAlarm:adjust_requires()
...
ModbusTcpClientAlarm.POST_validate_section_hook = ModbusTcpClientAlarm.adjust_requires
ModbusTcpClientAlarm.PUT_validate_section_hook = ModbusTcpClientAlarm.adjust_requires

function ModbusTcpClientAlarm:before_commit_hook()
...
ModbusTcpClientAlarm.POST_before_commit_hook = ModbusTcpClientAlarm.before_commit_hook
ModbusTcpClientAlarm.PUT_before_commit_hook = ModbusTcpClientAlarm.before_commit_hook

return ModbusTcpClientAlarm
