<template>
  ...
  >
    <vuci-named-section
      ...
      :uci-data="uciData"
      :data-key="type"
      :endpoints="[{ endpoint: `modbus/client/serial/servers/${parent}/alarms/config` }]"
      :name="section.id"
    ...
    >
      ...
      <vuci-form-item-input
        :uci-section="s"
        :label="$t('Timeout')"
        :help="$t('Time period for waiting of the serial device response (in seconds, 1-30).')"
        name="modbus_timeout"
        :depend="s.action === '2'"
        placeholder="5"
        initial="5"
      ...
      <vuci-form-item-input
        ...
        :depend="['5', '6', '15', '16'].includes(s.modbus_function) && s.action === '2'"
        :rules="validateWrite"
        required
      />
      <vuci-form-item-input
