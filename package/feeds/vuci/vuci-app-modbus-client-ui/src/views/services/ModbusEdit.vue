  ...
  >
    ...
    >
      <vuci-form-item-switch
        :uci-section="s"
        :label="$t('Enabled')"
        :help="$t('Check to enable this Modbus server configuration.')"
        name="enabled"
      />
      ...
      <vuci-form-item-input
        ...
        :uci-section="s"
        :label="$t('Address')"
        :help="$t('IP address or hostname of the server device.')"
        name="dev_ipaddr"
        placeholder="0.0.0.0"
        rules="host"
        required
      ...
      <vuci-form-item-switch
        :uci-section="s"
        :label="$t('Always reconnect')"
        :help="$t('Create new connection after every Modbus request.')"
        name="reconnect"
      />
      <vuci-form-item-input
        :uci-section="s"
        :label="$t('Number of timeouts')"
        :help="$t('Skip pending request and reset connection after number of request failures.')"
        name="skip_on_many_tmos"
        placeholder="0"
        initial="0"
        required
        :depend="s.reconnect === '0'"
        rules="irange(0,10)"
      />
      ...
      <vuci-form-item-input
        ...
        :uci-section="s"
        :label="$t('Delay')"
        :help="$t('Wait in milliseconds after connection initialization.')"
        name="delay"
        placeholder="0"
        initial="0"
        rules="irange(0,999)"
      ...
      <vuci-form-item-list
        ...
        :help="$t('Schedule request time (format: Hours:Minutes:Seconds).')"
        name="schedule"
        required
        :maxlines="255"
    ...
    <modbus-requests
      :section="section"
      :uci-data="uciData"
      :endpoint="`modbus/client/tcp/${section.id}/requests/config`"
    ...
    <vuci-typed-section
      ...
      :endpoints="[{ endpoint: `modbus/client/tcp/${section.id}/alarms/config` }]"
...
import EditForm from './ModbusAlarmEdit'
...
export default {
    ...
    return {
      ...
      alarmColumns: [
        ...
        },
        {
          name: 'action',
          label: this.$t('Action'),
          help: this.$t('Action triggered by this alarm.')
        },
        {
          name: 'enabled',
          label: this.$t('Enabled'),
          help: this.$t('If disabled, the alarm will not run.')
        }
      ]
    }
  },
  ...
  methods: {
    ...
    validateSchedule(value) {
      ...
      if (split.length !== 3) return { isValid: false, message: this.$t('Time format of hh:mm:ss is accepted.') }
    ...
    testRequest(requestData = {}) {
      const validations = ['delay', 'dev_ipaddr', 'port', 'server_id', 'timeout'].map(refName => {
      ...
      return Promise.all(validations)
        .then(valid => {
          ...
          const data = {
            ...
            dev_ipaddr: this.section.dev_ipaddr || badValues.push('IP address'),
            port: this.section.port || badValues.push('port'),
            delay: this.section.delay || badValues.push('delay')
          }

          if (badValues.length !== 0) throw new Error('missingValues', { cause: badValues })

          this.testDisabled = true
          return this.$axios.post(`/api/modbus/client/tcp/${this.section.id}/requests/actions/test_request`, { data })
        })
        .then(({ data }) => {
          let responseMessage = ''
    ...
    validateBeforeSave() {
      return new Promise((resolve, reject) => {
        ...
        const globalMissing = alarms.some(alarm => !alarm.action || !alarm.register)
        const modbusOptionsAvailable = alarms.some(alarm => {
          if (alarm.action !== '2') return false
          const { modbus_ip_addr, modbus_port, modbus_timeout, modbus_id, modbus_first_reg, modbus_reg_count } = alarm
          return (!modbus_ip_addr && modbus_port) || (!modbus_port && modbus_ip_addr) || !modbus_timeout || !modbus_id || !modbus_first_reg || !modbus_reg_count
        })
