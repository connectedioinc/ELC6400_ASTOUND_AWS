...
local tonumber, tostring, math, io = tonumber, tostring, math, io
...
IFACE_ERRORS = {
	...
	INVALID_ADDRESS			= ("IP address in invalid"),
	INVALID_GATEWAY			= ("Gateway address is invalid"),
	INVALID_LOCAL_ADDRESS	= ("Local IP address is invalid"),
	MISSING_ADDRESS			= ("IP address is missing"),
	MISSING_PEER_ADDRESS	= ("Peer address is missing"),
	NO_DEVICE				= ("Network device is not present"),
	NO_IFACE				= ("Unable to determine device name"),
	NO_IFNAME				= ("Unable to determine device name"),
	NO_WAN_ADDRESS			= ("Unable to determine external IP address"),
	NO_WAN_LINK				= ("Unable to determine upstream interface"),
	PEER_RESOLVE_FAIL		= ("Unable to resolve peer host name"),
	PIN_FAILED				= ("PIN code rejected")
}
...
local _interfaces, _bridge, _switch, _tunnel, _swtopo, _qinqdev, _physical_ifaces
...
function _wifi_sid_by_netid(netid)
...
function _wifi_sid_by_wid(wid)
	local sid
	_uci:foreach("wireless", "wifi-iface",
		function(s)
			if s.wifi_id and s.wifi_id == wid then
				sid = s[".name"]
				return false
			end
		end)
...
function _wifi_netid_by_sid(sid)
...
function init(cursor)
	...
	local iface
	for iface in nfs.dir("/sys/class/net") do
		local res = nfs.readlink("/sys/class/net/" .. iface)
		if res and not res:match("/virtual/") then
			table.insert(_physical_ifaces, iface)
		end
	end
	...
	local b, l
	if nfs.access("/usr/sbin/brctl") then
	...
	for switch, layout in pairs(boardinfo:get_switch()) do
			...
			for _, port in ipairs(layout.ports) do
				...
				then
					...
					if port.device then
						...
						if not netdevs[tostring(port.num)] then
...
function add_network(self, n, options)
	...
	if not oldnet and n and #n > 0 and n:match("^[a-zA-Z0-9_]+$") then
...
function get_devices(self)
	local _physical_ifaces = {}
	local nfs = require "nixio.fs"
	for iface in nfs.dir("/sys/class/net") do
		-- TODO: commented because it takes a lot of time to execute this code

		-- local interface = ntm:get_interface(iface)
		-- local dev_net = ""
		-- if interface then
		-- 	local ns = interface:get_networks()
		-- 	if #ns > 0 then
		-- 		dev_net = ns[1]:name()
		-- 		local n
		...
		-- 			dev_net = dev_net .. ", ".. n:name()
		-- 		end
		-- 	end
		-- end
		local res = nfs.readlink("/sys/class/net/" .. iface)
		-- local network = string.format("%s (%s)", iface, dev_net)
		-- if #dev_net == 0 then
		-- 	network = iface
		-- end
		...
		local tmp = {
			name = iface,
			virtual = (res and (not not res:match("/virtual/"))),
			dsa_cpu = nfs.access("/sys/class/net/" .. iface .. "/dsa")
			-- network = network
		}
		table.insert(_physical_ifaces, tmp)
	end
	_uci:foreach("network", "interface", function(s)
		local intf_type = _uci:get("network", "br_"..s[".name"], "type")
		if intf_type == "bridge" then
			local found = false
			for _, iface in ipairs(_physical_ifaces) do
				if iface.name == "br-" .. s[".name"] then
					found = true
					break
				end
			end
			if not found then
				table.insert(_physical_ifaces, { name = "br-" .. s[".name"], virtual = false, up = false })
			end
		end
	end)
	local ifaces = self:get_interfaces()
	for _, iface in ipairs(ifaces) do
		if (iface:type() == "vlan") then
			_physical_ifaces[#_physical_ifaces + 1] = { name = iface:name(), virtual = false, type = "VLAN" }
		end
	end

	local res = utl.ubus("network.device", "status") or {}
    for _, dev in ipairs(_physical_ifaces) do
        local found = false
        for devname, dev_ubus in pairs(res) do
            if dev.name == devname then
                utl.update(dev_ubus, dev)
                found = true
                break
            end
        end
	...
	for devname, dev in pairs(res) do
		dev.name = dev.name or devname
	end

	-- Adding default LAN and WAN devices to be visible all the time
	local def_lan_devs = boardinfo:get_default_lan_ifname()
	if def_lan_devs then
		for _, d in pairs(type(def_lan_devs) == "table" and def_lan_devs or {def_lan_devs}) do
			if not res[d] then
				res[d] = { name = d, virtual = false, type = d:match("%.") and "VLAN" or "Network device" }
			end
		end
	end

	local def_wan_dev = boardinfo:get_default_wan_ifname()
	if def_wan_dev and not res[def_wan_dev] then
		res[def_wan_dev] = { name = def_wan_dev, virtual = false, type = def_wan_dev:match("%.") and "VLAN" or "Network device" }
	end
...
function protocol.is_4_6(self)
	local matched = self:name():match("_4$") or self:name():match("_6$")
	local is_4_6_ = matched and uci:get("network", self:name()) == nil or nil
	return is_4_6_
end

function get_physical_ifaces(self)
	return _physical_ifaces
end
...
function get_interface(self, i)
	if _interfaces[i] or _wifi_iface(i) then
		...
		return netid and interface(netid) or nil
	end
end
...
function get_interfaces(self)
	...
	_uci:foreach("network", "interface",
		function(s)
			local ports = _uci:get("network", "br_"..s[".name"], "ports") or s.device
			for iface in utl.imatch(ports) do
	...
	-- for non RUTM
	local boardPorts = boardinfo:get_switch_ports()
	local vlanDevices = {}
	if boardPorts then
		for k, v in pairs(boardPorts) do
			if v.device then
				vlanDevices[v.device] = true
			end
		end
	end

	local readonly_vlans = boardinfo:get_readonly_vlans()
	local vlan0 = boardinfo:get_vlan0()
	_uci:foreach("network", "switch_vlan",
		function(s)
			...
			local pnum
			for pnum, _ in s.ports:gmatch("(%d+)([tu]?)") do
				...
				local pnum_number = pnum and tonumber(pnum) or nil
				if pnum_number and pnum_number >= 0 and pnum_number < 7 then
					local vid = tonumber(s.vid or s.vlan)
					if (vid ~= nil and vid > readonly_vlans and vid <= 4095) or (vlan0 and vid == 0) then
						local iface
						if vlanDevices.eth0 and vlanDevices.eth1 then
	...
	-- for RUTM
	_uci:foreach("network", "bridge-vlan",
		function(s)
			if type(s.vlan) ~= "string" or
			   type(s.device) ~= "string" or
				s.device ~= "vlan"
			then
				return
			end
			local intf = "vlan." .. s.vlan
			if not nfs[intf] then
				nfs[intf] = interface(intf)
			end
		end
	)
	
	--find q-in-q devces
	_uci:foreach("network", "device",
		function(s)
			if s.name and s.name:match("^%d+$") then
				nfs[s.name] = interface(s.name)
				_qinqdev[s.name] = s
			end
		end
	)
	...
	_uci:foreach("wireless", "wifi-iface",
		function(s)
			if s.device then
				for _, dev in pairs(type(s.device) == "table" and s.device or {s.device}) do
					num[dev] = num[dev] and num[dev] + 1 or 1
					local i = "%s.network%d" %{ dev, num[dev] }
					wfs[i] = interface(i)
				end
			end
		end)
...
function del_routes(self, inter_name)
	uci:foreach("network", "route", function(s)
		if s.interface == inter_name then
			uci:delete("network", s[".name"])
		end
	end)

	uci:foreach("network", "route6", function(s)
		if s.interface == inter_name then
			uci:delete("network", s[".name"])
		end
	end)
...
function get_wireless_status(self)
	return _wifi_state()
end
...
function get_wifinet_by_wid(self, wid)
	local sid = _wifi_sid_by_wid(wid)
	if sid then
		return wifinet(sid)
	end
end
...
function get_status_by_route(self, addr, mask)
	local _, object, metric, net_return, s_return
	for _, object in ipairs(utl.ubus()) do
		local net = object:match("^network%.interface%.(.+)")
		if net then
			local s = utl.ubus(object, "status", {})
			if s and s.route and s.metric then
				local rt
...
function get_topologies(self)
	local switches = {}
	local result = {}
	local topologies = self:get_switch_topologies() or {}
	_uci:foreach("network", "switch",
		function(x)
			...
			local swc = io.popen("swconfig dev %s help 2>/dev/null" % switch_name)
			if swc then
			...
			result = {
				...
				num_vlans = num_vlans,
				min_vid = min_vid,
				enable_vlan4k = enable_vlan4k,
				has_vlan4k = has_vlan4k,
...
-- finds switch from physical port (e.g "eth1")
-- returns switch_name (e.g. "switch0"), switch_object
function physical_port_switch(self, physical_port)
	local found_switch
	local switch_name, switch
	for switch_name, switch in pairs(self:get_switch_topologies()) do
		local port
		for _, port in ipairs(switch.ports) do
			if port.device == physical_port then
				return switch_name, switch
			end
		end
	end
end

-- finds switch uci section name from physical port (e.g "eth1")
function physical_port_switch_section(self, physical_port)
	local switch_name, _ = self:physical_port_switch(physical_port)
	local switch_section
	uci:foreach("network", "switch", function(s)
		if s["name"] == switch_name then
			switch_section = s[".name"]
			return false
		end
	end)
...
function get_type(self, name)
	local stat, tutil = pcall(require, "vuci.util_tlt")

	if stat then
		local ignore_list = { loopback = true, lan = true }
		local s = _uci:get_all("network", name)

		if not s then return end

		local net = self:get_network(name)
		...
		if not ignore_list[name] then
			...
			if device then
				local ifname =  device:name()
				_type = ifname and tutil.net_type(ifname, "ifname_reg")
			end

			if not _type then
				local proto = net:proto()
				_type = proto and tutil.net_type(proto, "proto")
			end
...
function protocol.bridge_ipaddr(self)
	local data = self:_ubus("data")
	local bridge_addr

	if type(data) == "table" and data.bridge_ipaddr ~= nil then
		bridge_addr = data.bridge_ipaddr
	end
...
function protocol.errors(self)
	...
	if type(errors) == "table" then
		for _, err in ipairs(errors) do
			...
			then
				rv = rv or { }
				rv[#rv+1] = IFACE_ERRORS[err.code] or "Unknown error (%s)" % err.code
			end
		end
	end
	return rv
...
function protocol.is_empty(self)
	if self:is_floating() then
		...
		if (self:_get("device") or ""):match("%S+") then
			empty = false

			local ports = _uci:get("network", "br_" .. self.sid, "ports")
			if _uci:get("network", "br_" .. self.sid) and not ports then
				empty = true
			end
		end
...
function protocol.get_interface(self)
	...
	elseif self:is_bridge() then
		...
		local devices = _uci:get("network", "br_"..self.sid) and _uci:get("network", "br_"..self.sid, "ports") or {_uci:get("network", self.sid, "device")}
		for _, ifn in pairs(devices) do
			ifn = ifn:match("^[^:/]+")
...
function protocol.lan_adminlink(self)
	local stat, dsp = pcall(require, "luci.dispatcher")
	return stat and dsp.build_url("admin", "network", "lan", self.sid)
end
...
function interface.bridge_ipaddr(self)
	return self.dev and self.dev.bridge_ipaddr or { }
end
...
function interface.type(self)
	for _, ifname in ipairs(utl.to_table(self.ifname)) do
		if ifname and ifname:byte(1) == 64 then
			return "alias"
		elseif self.wif or _wifi_iface(ifname) then
			return "wifi"
		elseif _bridge[ifname] then
			return "bridge"
		elseif _tunnel[ifname] then
			return "tunnel"
		elseif ifname:match("%.") then
			return "vlan"
		elseif _switch[ifname] then
			return "switch"
		elseif ifname:match("rmnet_data*") then
			return "mobile"
		elseif _qinqdev[ifname] then
			return "device"
		end
	end
	return "ethernet"
...
function interface.get_type_i18n(self)
	...
	elseif x == "vlan" then
		for _, ifname in ipairs(utl.to_table(self.ifname)) do
			if _switch[ifname] then
				return "Switch VLAN"
			else
				return "Software VLAN"
			end
		end
	elseif x == "tunnel" then
		return "Tunnel Interface"
	elseif x == "device" then
		return "Q-in-Q device"
	else
...
function interface.get_networks(self)
	if not self.networks then
		...
		for _, net in ipairs(_M:get_networks()) do
			for _, ifname in ipairs(utl.to_table(self.ifname)) do
				if net:contains_interface(ifname) or
					net:ifname() == ifname
				then
					nets[#nets+1] = net
				end
...
function wifidev.band(self)
    local hwmode = self:get('hwmode')
    if hwmode then
        if hwmode == "11a" or hwmode == "11bea" then
            return "5GHz"
        elseif hwmode == "11g" or hwmode == "11beg" then
            return "2.4GHz"
        else
            return ""
        end
...
function wifidev.get_i18n(self)
	...
	local m = self:standard()
	local name = self:name()
	if type(name) == "table" then
		local full_name = "%s %s Wireless Controller (" %{ t, m }
		...
		return "%s %s Wireless Controller (%s)" %{ t, m, name }
	end
end

function wifidev.standard(self)
	local m = {}
	local l = self:hwmodes()
	local hwmode = self:get('hwmode') or ""
	if l.be then table.insert(m, "be") end
	if l.ac and (hwmode == "11a" or hwmode == "11bea") then table.insert(m, "ac") end
	if l.ax then table.insert(m, "ax") end
	if l.a and hwmode == "11a" then table.insert(m, "a") end
	if l.b and (hwmode == "11g" or hwmode == "11beg") then table.insert(m, "b") end
	if l.g and (hwmode == "11g" or hwmode == "11beg") then table.insert(m, "g") end
	if l.n then table.insert(m, "n") end

	return "802.11" .. table.concat(m, "/")
end

function wifidev.is_pending(self)
	if _ubuswificache[self.sid] then
		return (_ubuswificache[self.sid].pending == true)
	end
...
function wifinet.ifname(self)
	local devices = self:get_device()
	if type(devices) == "table" and #devices > 0 then
		local ifnames = {}
		for _, radiostate in pairs(_wifi_state()) do
			if type(radiostate) == "table" and type(radiostate.interfaces) == "table" then
				for _, netstate in ipairs(radiostate.interfaces) do
					if type(netstate) == "table" and type(netstate.section) == "string" and netstate.section == self.sid then
						table.insert(ifnames, netstate.ifname)
					end
				end
			end
		end
...
function wifinet.get_device(self)
	...
	if type(dev) == "table" then
		local wifidevs = {}
		for _, d in pairs(dev) do
			wifidevs[#wifidevs+1] = wifidev(d)
		end
		return wifidevs
	end
...
function wifinet.active_mode(self)
	return self:get("mode") or self.iwinfo.mode or self:ubus("net", "config", "mode") or "ap"
end
...
function wifinet.active_bssid(self)
	return self:get("bssid") or self.iwinfo.bssid or self:ubus("net", "config", "bssid")
end
	...
	if type(devices) == "table" and #devices > 0 then
		...
		for _, radiostate in pairs(_wifi_state()) do
			if type(radiostate) == "table" and type(radiostate.interfaces) == "table" then
				for _, netstate in ipairs(radiostate.interfaces) do
					if type(netstate) == "table" and type(netstate.section) == "string" and netstate.section == self.sid then
						local iwinfo = _wifi_iwinfo_by_ifname(netstate.ifname)
						for k, v in pairs(iwinfo and iwinfo.assoclist or {}) do
							assoclist[k] = v
						end
					end
				end
...
function wifinet.channel(self)
	return self.iwinfo.channel or self:ubus("dev", "config", "channel") or (self:get("channel") and tonumber(self:get("channel")) or nil)
end
...
function wifinet.signal_percent(self)
	...
	local qc, qm = 0, 0
	if type(ifname) == "table" and #ifname > 1 then
		local iwinfo = require "iwinfo"
		for _, ifn in ipairs(ifname) do
			local iwtype = iwinfo.type(ifn)
			if iwtype then
				local iwinfo_data = iwinfo[iwtype]
				local quality = iwinfo_data and iwinfo_data.quality(ifn)
				local quality_max = iwinfo_data and iwinfo_data.quality_max(ifn)
				if quality and quality_max then
					qc = quality
					qm = quality_max
					break
				end
		...
		qc = self.iwinfo.quality or 0
		qm = self.iwinfo.quality_max or 0
	end
...
function wifinet.shortname(self)
	local ssid = self:active_ssid() or self:active_bssid() or self:id() or ""
	return "%s %q" %{
		self:active_mode(),
		ssid
	}
...
_M:register_protocol("connm")
_M:register_protocol("dhcpv6")
_M:register_protocol("pppoe")

-- load protocol extensions
local exts = nfs.dir(utl.libpath() .. "/network")

if exts then
	local ext
	for ext in exts do
		if ext:match("%.lua$") then
			require("vuci.network." .. ext:gsub("%.lua$", ""))
		end
	end
end
