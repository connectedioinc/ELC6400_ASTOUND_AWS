...
function OpkgPkg:_install_packages_offline()
	...
	local ipk_files = {self.pkg_dir .. "/" .. self.ipk_file:match("([^:]+):")}
	for _, dep in ipairs(self.ipk_deps) do
		ipk_files[#ipk_files + 1] = self.pkg_dir .. "/" .. dep:match("([^:]+):")
	end

	local code = o_utils.opkg_call("install", ipk_files, o_utils.OPKG_DATA, "--tmp-dir", self.pkg_dir)
	if code ~= 0 then
		return false, ERR_CODES.INSTALL_FAIL
	end
	return true
...
function OpkgPkg:_check_package_arch_and_hw()
	local package_arch = self:get("Router")

	local ok, hw_arch = o_utils.check_package_arch(package_arch)
	if not ok then
		util.perror("Different architectures: %s != %s" % {hw_arch, package_arch or ""})
		return false
	end
...
function OpkgPkg:_check_size_offline()
	...
	for pkg_file_name in fs.dir(self.pkg_dir) do
		if pkg_file_name ~= "control_dir" and pkg_file_name ~= "main" and pkg_file_name ~= "main.sig" then
			util.exec("/bin/tar -xzC%s/control_dir -f %s/%s" % {self.pkg_dir, self.pkg_dir, pkg_file_name})
			local pkg_info_text = util.exec("zcat %s/control_dir/control.tar.gz" % self.pkg_dir)

			local pkg = OpkgPkg(pkg_info_text)
			if not pkg:is_installed() then
				local installed_size = pkg:get("Installed-Size")
