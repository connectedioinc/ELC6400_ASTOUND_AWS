...
local OPKG_CMD = "opkg --force_feeds /etc/opkg/teltonikafeeds.conf -f %s --force-removal-of-essential-packages " % OPKG_CONF_FILE
local DEST_ROOT = (fs.readfile(OPKG_CONF_FILE):match("dest%s+root%s+(%S+)") or ""):gsub("/$", "") -- remove trailing slash
local OPKG_DATA = { "--force_feeds", "/etc/opkg/teltonikafeeds.conf", "--conf", OPKG_CONF_FILE, "--force-removal-of-essential-packages"}
...
function o_utils.get_installed_pkg_info_text(pkg_name)
	...
	if not (pkg_text or ""):find("\nRouter: ", nil, true) then return nil end
	return pkg_text
end
...
---@param package_router_name string?
---@return boolean
---@return string hw_arch device's family name
function o_utils.check_package_arch(package_router_name)
	local hw_arch = board:get_family_name()
	if hw_arch ~= package_router_name then
