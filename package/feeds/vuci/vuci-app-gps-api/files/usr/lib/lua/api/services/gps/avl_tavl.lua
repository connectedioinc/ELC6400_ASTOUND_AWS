...
local TAVL = ConfigService:new({ increment_name = true })

local TAVLRules = TAVL:section("avl", "tavl_rule")

	local opt_name = TAVLRules:option("name")
	function opt_name:validate(value)
		return self.dt:check_array(value, gps_utils.list_available_tavl_names(true))
	end

	local opt_type = TAVLRules:option("type")
	...
	local opt_enabled = TAVLRules:option("enabled")
